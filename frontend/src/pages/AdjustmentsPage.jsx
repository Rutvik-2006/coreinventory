...
import { stockApi, productsApi, warehousesApi, movesApi } from "../utils/api";
...
export default function AdjustmentsPage() {
  ...
  const loadHistory = () => {
    movesApi
      .list({ type: "adjustment", limit: 20 })
      .then((r) => setHistory(r.data))
      .catch((err) => console.error("Failed to load history:", err)); // Added catch to log errors safely
  };
    ...
    productsApi.get(form.product_id).then((r) => {
      const snap = r.data.stock_by_location?.find((s) => {
        ...
        return locations.find(
          (l) => l.id === form.location_id && l.name === s.location_name,
        );
      });
  ...
  const validate = () => {
    ...
    if (form.new_qty === "" || Number(form.new_qty) < 0)
      e.new_qty = "Enter a valid quantity (≥ 0)";
    return e;
  };
  ...
  const diff =
    currentStock !== null && form.new_qty !== ""
      ? Number(form.new_qty) - currentStock
      : null;
        ...
        <div className="card" style={{ width: 400, flexShrink: 0 }}>
          ...
          >
            <div className="form-group">
              ...
              <select
                className={`input select ${errors.product_id ? "error" : ""}`}
            ...
            <div className="form-group">
              ...
              <select
                className={`input select ${errors.location_id ? "error" : ""}`}
                value={form.location_id}
                onChange={setF("location_id")}
                disabled={!canAdjust}
              >
              ...
              {errors.location_id && (
                <span className="error-msg">{errors.location_id}</span>
              )}
            </div>
            ...
            <div className="form-group">
              ...
              <input
                className={`input ${errors.new_qty ? "error" : ""}`}
                type="number"
                min="0"
                step="any"
                value={form.new_qty}
                onChange={setF("new_qty")}
                placeholder="0"
                disabled={!canAdjust}
              />
              {errors.new_qty && (
                <span className="error-msg">{errors.new_qty}</span>
              )}
              {diff !== null && (
                <span
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                value={form.notes}
                onChange={setF("notes")}
                placeholder="Damaged goods, count discrepancy…"
            ...
            >
              {saving ? (
                <>
                  <div className="spinner" /> Adjusting…
                </>
              ) : (
                "Apply Adjustment"
              )}
              ...
              <tbody>
                {history.length === 0 ? (
                  ...
                  history.map((h) => (
                    <tr key={h.id}>
                      <td>
                        <span className="ref-chip">{h.reference}</span>
                      </td>
                      <td style={{ fontWeight: 500 }}>{h.product_name}</td>
                      <td
                        className={`mono ${Number(h.qty_done) >= 0 ? "" : ""}`}
                      >
                        {h.direction === "out" ? "-" : "+"}
                        {Number(h.qty_done).toLocaleString()}{" "}
                        {h.unit_of_measure}
                      </td>
